...
class SVDAlgorithm:
    ...
    def eigenvalue_decomposition(self, M):
        """
        ...
        Output:
        Returns list - sorted_eval, sorted_evec
        sorted_eval - list of sorted eigen_values
        sorted_evec - numpy matrix containing eigen vectors
        """
        ...
        sorted_evec = np.zeros_like(eigen_vectors)
        for i in range(len(sorted_eval)):
            sorted_evec[:, i] = eigen[sorted_eval[i]]

        sorted_evec = sorted_evec[:, :len(sorted_eval)]  # Removing zeroed eigen vectors
    ...
    def svd(self, M, dimension_reduction=1.0):
        ...
        eigen_values_u, U = self.eigenvalue_decomposition(np.dot(M, M.T))
        eigen_values_v, V = self.eigenvalue_decomposition(np.dot(M.T, M))
